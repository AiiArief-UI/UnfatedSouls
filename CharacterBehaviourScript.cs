...
public class CharacterBehaviourScript : MonoBehaviour {
	...
	void Update () {
		...
		if ((Input.GetMouseButtonDown(0)) && shootFlag) {
			...
			arah = new Vector2 ( Camera.main.ScreenToWorldPoint( Input.mousePosition).x-gameObject.transform.position.x,Camera.main.ScreenToWorldPoint(Input.mousePosition).y- gameObject.transform.position.y );
			...
			GameObject bullet=(GameObject)Instantiate(bulletPrefab,bulletPos,bulletPrefab.transform.rotation);
			bullet.GetComponent<Rigidbody2D>().velocity= (arah *bulletSpeed);
			shootFlag=false;
	...
	void FixedUpdate () {
		...
		if (!((direction == 1 && facingRight && checkWall ()) || (direction == -1 && !facingRight && checkWall ()))&&direction!=0) {
			rigidbody2d.velocity = new Vector2 (moveForce * direction,rigidbody2d.velocity.y);
